...
if [ "$OS" = "RHEL" ]; then
    sudo $PACKAGE_MANAGER install -y wget curl tar
    sudo $PACKAGE_MANAGER install -y epel-release
    sudo $PACKAGE_MANAGER install -y golang
else
    sudo $PACKAGE_MANAGER update
    sudo $PACKAGE_MANAGER install -y wget curl tar golang
fi

# Создание пользователей и директорий
echo "Создание пользователей и директорий..."
...
if [ "$OS" = "RHEL" ]; then
    sudo $PACKAGE_MANAGER install -y https://dl.grafana.com/oss/release/grafana-10.0.3-1.x86_64.rpm
else
